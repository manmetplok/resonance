...
fn tonic_chord(scale: Scale) -> Chord {
    let quality = match scale.mode {
        ...
        // Chromatic has no diatonic tonic quality; default to a major
        // triad on the root so the chord track still seeds a usable chord.
        Mode::Chromatic => ChordQuality::Maj,
    };
