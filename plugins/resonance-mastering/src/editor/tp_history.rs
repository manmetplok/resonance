//! Rolling true-peak hold trace.

use wayland_plugin_gui::egui;

use super::theme;
use crate::viz::MasteringViz;

const FLOOR_DBTP: f32 = -40.0;
const CEILING_DBTP: f32 = 3.0;

pub fn draw(painter: &egui::Painter, rect: egui::Rect, viz: &MasteringViz) {
    painter.rect_filled(rect, 2.0, theme::PANEL);
    painter.rect_stroke(
        rect,
        2.0,
        egui::Stroke::new(1.0, theme::BORDER),
        egui::StrokeKind::Inside,
    );

    painter.text(
        egui::pos2(rect.left() + 8.0, rect.top() + 4.0),
        egui::Align2::LEFT_TOP,
        "True-peak history  (5 s)",
        egui::FontId::monospace(10.0),
        theme::TEXT_DIM,
    );

    // 0 dBTP ceiling.
    let zero_y = dbtp_to_y(0.0, rect);
    painter.line_segment(
        [
            egui::pos2(rect.left(), zero_y),
            egui::pos2(rect.right(), zero_y),
        ],
        egui::Stroke::new(1.0, theme::DANGER),
    );

    let n = crate::viz::TP_HISTORY_LEN;
    let samples: Vec<f32> = viz.tp_history.iter_chrono().collect();

    if samples.is_empty() {
        return;
    }
    let step = rect.width() / (n as f32 - 1.0).max(1.0);
    let mut prev: Option<egui::Pos2> = None;
    for (i, &v) in samples.iter().enumerate() {
        if !v.is_finite() || v <= FLOOR_DBTP {
            prev = None;
            continue;
        }
        let x = rect.left() + i as f32 * step;
        let y = dbtp_to_y(v, rect);
        let p = egui::pos2(x, y);
        if let Some(pp) = prev {
            painter.line_segment([pp, p], egui::Stroke::new(1.0, theme::WARN));
        }
        prev = Some(p);
    }
}

fn dbtp_to_y(dbtp: f32, rect: egui::Rect) -> f32 {
    let norm = ((dbtp - FLOOR_DBTP) / (CEILING_DBTP - FLOOR_DBTP)).clamp(0.0, 1.0);
    rect.bottom() - norm * (rect.height() - 16.0) - 4.0
}
