...
/// Unipolar slider: `value_unit` 0..1. Returns the new value if dragged.
pub fn slider_unipolar(ui: &mut egui::Ui, width: f32, value_unit: f32) -> Option<f32> {
    draw(ui, width, value_unit, false, false)
}

/// Bipolar slider: `value_signed` -1..1. Returns the new bipolar value if dragged.
pub fn slider_bipolar(ui: &mut egui::Ui, width: f32, value_signed: f32) -> Option<f32> {
    let unit = (value_signed + 1.0) * 0.5;
    draw(ui, width, unit, true, false).map(|u| u * 2.0 - 1.0)
}

/// Bipolar slider with the warm (amber) palette — used for mic balance.
pub fn slider_bipolar_warm(ui: &mut egui::Ui, width: f32, value_signed: f32) -> Option<f32> {
    let unit = (value_signed + 1.0) * 0.5;
    draw(ui, width, unit, true, true).map(|u| u * 2.0 - 1.0)
}
...
fn draw(
    ...
    let primary = if warm { theme::WARM } else { theme::ACCENT };

    if bipolar {
        let center_x = rect.center().x;
        // Mirrors the design handoff:
        //   • accent (lavender) sliders fill warm on the negative side
        //     so the polarity reads at a glance;
        //   • warm sliders fill warm on both sides — they're already
        //     polarity-coloured.
        if thumb_x >= center_x {
            ...
            painter.rect_filled(fill, 1.5, primary);
        } else {
            let fill = egui::Rect::from_min_max(
