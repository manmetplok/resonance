...
When you need to rebase a branch in isolation, create a scratch worktree with `git worktree add`
and remove it (`git worktree remove`) when you're done — never check a feature branch out in the
main tree.
...
- After landing (and once per pass generally), sweep up branches: `ba branch prune`. It deletes
  `ba/*` branches already merged into integration and prunes dead worktrees — never touching
  unmerged work. It also reports **stale unmerged** `ba/*` branches: cross-check each against its
  todo/epic — if that work is `done`/`cancelled` yet the branch never merged, it's an orphan, so
  investigate (re-submit it, or escalate). Run `ba branch list` first if you want to see the state.
...
2. In a scratch worktree (create one with `git worktree add`), check the branch out, rebase it on
   the target, **resolve the conflicts** (edit the files, keeping both sides' intent), then commit;
