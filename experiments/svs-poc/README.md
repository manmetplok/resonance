# svs-poc — DiffSinger ONNX inference, isolated

Renders a DiffSinger `.ds` JSON score into a WAV file by running the voicebank's
acoustic ONNX → vocoder ONNX pipeline. This is a **proof of concept** living in
`experiments/svs-poc/`; no other Resonance crate depends on it.

## TL;DR — get singing in under 5 minutes

```bash
# 1. Fetch TIGER (English DiffSinger) + its companion vocoder (~580 MB total).
experiments/svs-poc/scripts/download_models.sh

# 2. Render the bundled sample .ds file.
cargo run -p svs-poc --release -- \
  --ds-file        experiments/svs-poc/models/sample/hello_tiger.ds \
  --acoustic-config experiments/svs-poc/models/singer/extracted/dsacoustic/dsconfig.yaml \
  --vocoder-config  experiments/svs-poc/models/singer/extracted/dsvocoder/vocoder.yaml \
  --out            experiments/svs-poc/models/sample/hello_tiger.wav \
  --speaker        tiger_disco \
  --speedup        20

# 3. Listen.
xdg-open experiments/svs-poc/models/sample/hello_tiger.wav
```
...
```
cargo build -p svs-poc                # CPU only
cargo build -p svs-poc --release
cargo build -p svs-poc --features cuda --release   # requires CUDA toolchain
cargo build -p svs-poc --features rocm --release   # requires ROCm + matching ort binary
```
...
`scripts/download_models.sh` downloads and extracts into `experiments/svs-poc/models/`:
...
```
cargo run -p svs-poc --bin probe --release -- path/to/model.onnx
```
...
PoC's stage modules use runtime introspection of these names to decide which
optional inputs to populate.

## Tests

```
cargo test -p svs-poc                                                          # parser-only smoke
SVS_POC_VOICEBANK_DIR=experiments/svs-poc/models/singer/extracted/dsacoustic \
  SVS_POC_DS_FILE=experiments/svs-poc/models/sample/hello_tiger.ds \
  SVS_POC_SPEAKER=tiger_disco \
  cargo test -p svs-poc end_to_end_render -- --nocapture
```
...
```
rm -rf experiments/svs-poc/
# then edit Cargo.toml and drop "experiments/svs-poc" from the workspace members list
```

That's the entire removal procedure. No other Resonance crate has a dependency
on this one (verify with `cargo tree --invert -p svs-poc`).
...
│   ├── bin/probe.rs           # ONNX I/O inspector
